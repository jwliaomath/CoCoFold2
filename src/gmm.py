...
Optional peak_3d treats atom_weights as the 3-D peak before common scaling:
q = (2*pi)**1.5 * sqrt(det(covariance)) * atom_weights.
...
class GaussianProjector(nn.Module):
    ...
    def __init__(self, initial_weights, kernel="legacy", sigma_init=None,
        ...
        allowed = (("peak_2d",) if kernel == "legacy" else
                   ("reference_mass", "peak_3d") if kernel == "anisotropic" else
        ...
        self.checkpoint_peak2d = bool(checkpoint_peak2d)
    ...
    def set_learning_enabled(self, enabled):
        """Toggle amplitude and width parameters while preserving coordinate gradients."""
        if not isinstance(enabled, bool):
            raise ValueError('GMM learning flag must be boolean')
        for parameter in self.amplitude_parameters() + self.shape_parameters():
    ...
    def render_raw(self, projected_coordinates, rotation, global_origin, resolution,
        ...
        if self.amplitude_convention == "peak_2d":
            ...
            renderer = (sum_of_gaussians_2d_torch_checkpointed if self.checkpoint_peak2d
                        else sum_of_gaussians_2d_torch)
            image = renderer(
            ...
            cov3 = self.covariance()
            cov2 = project_gaussian_covariances(cov3, rotation)
            if self.amplitude_convention == "peak_3d":
                # Use the actual covariance INCLUDING sigma_floor**2 * I.
                # Its Cholesky diagonal product is sqrt(det(cov3)). Keep this
                # differentiable: changing widths must also change the mass.
                lower = torch.linalg.cholesky(cov3.to(device=weights.device, dtype=cov2.dtype))
                sqrt_det = lower.diagonal(dim1=-2, dim2=-1).prod(dim=-1)
                mass = (2 * math.pi)**1.5 * weights * sqrt_det
            else:
                # Preserve the existing reference_mass arithmetic exactly.
    ...
    def forward(self, atoms_coord, rotation, trans, resolution, density_center,
        ...
        if self.amplitude_convention == "peak_2d":
            ...
            renderer = pdb2img_peak2d_checkpointed if self.checkpoint_peak2d else pdb2img
            return renderer(atoms_coord, resolution, self.atom_weights, rotation,
    ...
    def config(self):
        return {
            ...
            "checkpoint_peak2d": self.checkpoint_peak2d,
...
def add_gmm_arguments(parser):
    ...
    parser.add_argument("--gmm-atom-chunk-size", type=int, default=1000,
    ...
    parser.add_argument("--gmm-checkpoint-peak2d", action=argparse.BooleanOptionalAction,
