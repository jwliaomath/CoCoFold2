...
class Protenix(nn.Module):
    ...
    def get_pairformer_output(
        ...
        return_pairformer_last_input: bool = False,
    ) -> tuple[torch.Tensor, ...]:
        """
        ...
        pairformer_last_input_dict: Optional[dict[str, Any]] = None
        ...
        for cycle_no in range(N_cycle):
            with torch.set_grad_enabled(
                ...
                if return_pairformer_last_input and cycle_no == (N_cycle - 1):
                    pairformer_last_input_dict = {
                        "s": s.detach().clone(),
                        "z": z.detach().clone(),
                        "cycle_no": int(cycle_no),
                        "input_is_after_recycle_template_msa": True,
                        "input_is_before_pairformer_stack": True,
                        "bias_name": "pairformer_last_input_z_bias",
                        "note": (
                            "input to the final PairformerStack, after "
                            "recycle/template/MSA, before pairformer_stack"
                        ),
                    }
        ...
        if return_pairformer_last_input:
            return s_inputs, s, z, pairformer_last_input_dict
        return s_inputs, s, z
    ...
    def _main_inference_loop(
        ...
        save_pairformer_last_input = bool(
            getattr(self.configs, "save_pairformer_last_input", False)
        )
        pairformer_last_input_dict = None

        if save_pairformer_last_input:
            s_inputs, s, z, pairformer_last_input_dict = self.get_pairformer_output(
                input_feature_dict=input_feature_dict,
                N_cycle=N_cycle,
                inplace_safe=inplace_safe,
                chunk_size=chunk_size,
                mc_dropout=mc_dropout,
                return_pairformer_last_input=True,
            )
        else:
        ...
        if self.configs.output_model_dir is not None:
            diffusion_data = {
                "pred_dict":pred_dict,
                "model_state":self.diffusion_module.state_dict(),
                "input_feature_dict":input_feature_dict,
                "s_inputs":s_inputs,
                "s_trunk":s,
            ...
            if save_pairformer_last_input:
                if pairformer_last_input_dict is None:
                    raise RuntimeError(
                        "save_pairformer_last_input=True but final PairformerStack input was not captured."
                    )
                diffusion_data["pairformer_last_input"] = pairformer_last_input_dict
                diffusion_data["pairformer_stack_state"] = self.pairformer_stack.state_dict()
